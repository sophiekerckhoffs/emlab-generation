...
headlessModelRunName<-"test1"
noOfTicks<-40
scenario <- "/home/sophie/emlab-generation/emlab-generation/src/main/resources/scenarios/scenarioS-OneCountryStorage.xml" #Scenario name can be set here, incl. the xml ending
dryRun<-FALSE
headlessRun<-FALSE
source("rConfig.R")
source("simulation.R")
...
if(exists("scenario") & !dryRun){
  if(!is.null(scenario)) message(paste("Load Scenario",scenario,loadScenario(scenario)))
}
...
createPostRunPlots(runResult, list(
  list("Avg_El_PricesinEURpMWh_Country A", "Electr. Price [Eur/MWh]", "Average Eletricity Price in Country A"),
  list("Avg_El_PricesinEURpMWh_Country B", "Electr. Price [Eur/MWh]", "Average Eletricity Price in Country B"), 
  list("CO2Emissions_inTonpA",expression(paste(CO[2], " Emissions [Ton/a]", sep="")), "CO$_2$ Emissions"), 
  list("CO2Auction", expression(paste(CO[2]," Price [EUR/t]", sep="")), "CO$_2$ Price")))
@
